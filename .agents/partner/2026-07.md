...
description: >-
  The Partner API provides access to data in the Partners Dashboard. Learn how
  to get started using efficient GraphQL queries.
api_version: 2026-07
source_url:
  html: 'https://shopify.dev/docs/api/partner/2026-07'
  md: 'https://shopify.dev/docs/api/partner/2026-07.md'
api_name: partner
api_type: graphql
---
...
* Your organization ID. You can find this in the URL of the Partners Dashboard when you're logged in. For example: `https://partners.shopify.com/organization-id/api/2026-07/graphql.json`
...
```bash
curl -X POST \
  https://partners.shopify.com/{organization_id}/api/2026-07/graphql.json \
...
* View financials: This permission is required to access Transaction resources and to cancel Shopify App Pricing subscriptions. Transaction resources represent all of the transactions that impact your Partner earnings.
...
## https://partners.shopify.com/{org\_id}/api/2026-07/graphql.json

```bash
# Get the ID and user name of your 20 most recent unread messages
curl -X POST "https://partners.shopify.com/{org_id}/api/2026-07/graphql.json" \
  -H "Content-Type: application/json" \
...
The Partner API includes queries and mutations that support [Shopify App Pricing](https://shopify.dev/docs/apps/launch/billing/shopify-app-pricing), the managed pricing system for public apps.

* **[Active Subscription](https://shopify.dev/docs/api/partner/active-subscription)**: Retrieve the current managed pricing subscription for a specific shop, including plan, billing period, billing cycle, subscription items (with pricing and usage), discounts, and pending updates. Requires the **Manage apps** permission.
* **[App Subscription Cancel](https://shopify.dev/docs/api/partner/app-subscription-cancel)**: Cancel the active Shopify App Pricing subscription for a specific shop and app. Requires the **View financials** permission.
* **[Historical Events](https://shopify.dev/docs/api/partner/historical-events)**: Retrieve a paginated timeline of installs, uninstalls, charges, earnings, subscription changes, and credits across your apps and themes. Supports filtering by shop, subject, event type, and date range. Requires the **Manage apps** permission.
